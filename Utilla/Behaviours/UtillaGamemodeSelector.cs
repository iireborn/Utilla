...
namespace Utilla.Behaviours
{
    ...
    internal class UtillaGamemodeSelector : MonoBehaviour
    {
        ...
        public async void Awake()
        {
            ...
            if (Layout is null)
            {
                Logging.Error($"{nameof(UtillaGamemodeSelector)} spawned without a {nameof(GameModeSelectorButtonLayout)}; destroying selector.");
                Destroy(this);
                return;
            }
            ...
            // Game can exit to a different scene while this component is still awaiting; bail out if we were disabled.
            while (Layout.currentButtons.Count == 0)
            {
                if (this is null || !this) return;
                await Task.Yield();
            }
