...
namespace Utilla.Behaviours
{
    internal class UtillaNetworkController : MonoBehaviourPunCallbacks
    {
        ...
        {
            if (ApplicationQuittingState.IsQuitting || !NetworkSystem.Instance.InRoom || NetworkSystem.Instance.GameModeString is not string gameMode || string.IsNullOrEmpty(gameMode)) return;

            GameModeUtils.CurrentGamemode = GameModeUtils.FindGamemodeInString(gameMode);

            // If we never recorded the join (e.g. Utilla loaded mid-session), treat this as a fresh join instead of a switch.
            if (lastRoom is null)
            {
                lastRoom = new Events.RoomJoinedArgs { isPrivate = NetworkSystem.Instance.SessionIsPrivate, Gamemode = gameMode };
