...
namespace Utilla.Patches
{
    ...
    internal static class GorillaComputerPatches
    {
        ...
        internal static bool InitializeModePatch(GorillaComputer __instance)
        {
            ...
            // If the transpiler could not extract the preference key, fall back to the original method
            // rather than passing null into PlayerPrefs.GetString (which would throw).
            if (string.IsNullOrEmpty(ModePreferenceKey))
            {
                Logging.Warning("Could not resolve mode preference key; deferring to original game behavior.");
                return true;
            }
